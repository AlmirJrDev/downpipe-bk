...
/** Publicações marcadas com um rolê — usado por GET /events/:eventId/posts. */
export async function listByEvent(req: Request, res: Response, next: NextFunction) {
  try {
    const { eventId } = eventIdParamSchema.parse(req.params);
    const pagination = parsePagination(req);
    const { posts, total } = await postsService.listByEvent(eventId, pagination, req.user?.id);
    sendPaginated(res, posts, buildPaginationMeta(pagination.page, pagination.limit, total));
  } catch (err) {
    next(err);
  }
...
/** Dono do carro aceita ou recusa uma marcação pendente. */
export async function respondCarTag(req: Request, res: Response, next: NextFunction) {
  try {
    if (!req.user) throw AppError.unauthorized();
    const { id } = postIdParamSchema.parse(req.params);
    const { accept } = carTagResponseSchema.parse(req.body);
    const post = await postsService.respondToCarTag(id, req.user.id, accept);
    sendSuccess(res, post);
  } catch (err) {
    next(err);
  }
